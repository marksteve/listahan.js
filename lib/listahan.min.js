...
q).css("overflow-y","scroll")};v={};p=d.menu;f=0;for(n=p.length;f<n;f++)l=p[f],v[l.id]=l;t=function(a){var b,j,g,f;v[a.id]=a;g=k[a.parent];g||(g=k[a.parent]=$("<ul/>").addClass(d.submenuClass),f=v[a.parent],j=(null!=f?f.parent:void 0)||d.root,b=k[j],j=$("#"+a.parent,k[j]),j=$.proxy(d.menuItemParent,j),j(f||"root",b,k));b=$("<li/>").attr("id",a.id).data("item",a).on("mouseenter",function(){var c,b,f;clearTimeout(w);c=$(this);for(b=a.parent;b!=null;){c.addClass("active").siblings().removeClass("active");
b=$("li#"+b,e).addClass("active");b=b.size()?b.data("item").parent:null}e.data("active",c.attr("id"));f=$.proxy(d.menuItemActive,this);w=setTimeout(function(){o(a.parent);m(a,c);f(a,g,k)},d.showDelay)}).on("click",function(b){$.proxy(d.menuItemClick,this)(a,g,k,b)}).appendTo(g);$.proxy(d.menuItemAdd,b)(a,g,k)};p=d.menu;f=0;for(n=p.length;f<n;f++)l=p[f],t(l);x=function(a){$("#"+a.id,k[a.parent]).remove();a=k[a.id];null!=a&&($("li",a).each(function(){var a;a=$(this);x(a.data("item"));return a.remove()}),
a.remove())};e.css("position","absolute").hide();s=k[d.root].css("position","absolute");e.data("options",d);e.data("$submenus",k);d.menuInit(e);return e.on("add.listahan",function(a,b){t(b)}).on("remove.listahan",function(a,b){x(b)}).on("clear.listahan",function(a,b){null!=k[b]&&k[b].empty()}).on("show.listahan",function(){e.show();o(d.root);s.is(":visible")||e.append(s);$.proxy(d.menuOpen,this)(e);["up","down","left","right"].forEach(function(a){return Mousetrap.bind(a,function(b){b.preventDefault();
